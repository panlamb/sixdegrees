  ...
  return (
    <PageWrapper>
      ...
      <div className="px-5 pb-28">
        ...
        {!selected && (
          <div>
            <div className="font-mono text-[9px] uppercase tracking-[0.15em] text-[#888] mb-3">
              ...
              <div className="mt-2 flex flex-col gap-px">
                {results.map(user => (
                  <div
                    ...
                    className="flex items-center gap-3 px-4 py-3.5 bg-[#1e1e1e] rounded-sm cursor-pointer hover:bg-[#161616] transition-colors"
                  >
                    <Avatar initials={user.name.slice(0, 2).toUpperCase()} size={38} active />
                    <div className="flex-1">
                      <div className="font-serif text-[14px] text-white">{user.name}</div>
                      {user.city && <div className="font-mono text-[9px] text-[#777] mt-0.5">{user.city}</div>}
                    </div>
                  </div>
                ))}
              </div>
            ...
            {query.length > 1 && results.length === 0 && (
              <div className="mt-3 px-4 py-3 bg-[#1e1e1e] border border-[#333] rounded-sm">
                <div className="font-mono text-[9px] text-[#777] mb-1">NOT ON SIX° YET</div>
                <div className="font-serif text-[14px] text-white">{query}</div>
              </div>
            )}
          </div>
        )}
          ...
          <div>
            <div className="flex items-center gap-3 bg-[#0d1a00] border border-lime/20 rounded-sm px-4 py-3 mb-6">
              ...
              <div className="flex-1">
                ...
                <div className="font-serif text-[15px] text-white">{targetName}</div>
              </div>
              <button
                onClick={() => { setSelected(null); setQuery(''); setLinks([]); setResults([]); }}
                className="font-mono text-[10px] text-[#888] hover:text-[#888]"
              >✕</button>
            </div>

            <div className="bg-[#1e1e1e] border border-[#1a1a1a] rounded-sm p-4 mb-4">
              <div className="font-mono text-[9px] uppercase tracking-[0.15em] text-[#888] mb-4">YOUR CHAIN</div>

              <div className="flex items-center gap-3 mb-1">
                <Avatar initials="PK" size={34} active />
                <div className="flex-1">
                  <div className="font-mono text-[11px] text-white">You</div>
              ...
              {links.map((link, i) => (
                <div key={i}>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      <div className="font-mono text-[11px] text-white">{link.name}</div>
                      <div className="font-mono text-[8px] text-[#c8f05a88] mt-0.5 uppercase tracking-[0.1em]">pending</div>
                    </div>
                    <button onClick={() => removeLink(i)} className="font-mono text-[10px] text-[#888] hover:text-[#888]">✕</button>
                  </div>
                </div>
              ))}
              ...
              <div className="flex items-center gap-3 opacity-25">
                <div className="w-[34px] h-[34px] rounded-full border border-dashed border-[#444] flex items-center justify-center text-[#777] text-lg flex-shrink-0">?</div>
                <div className="font-mono text-[11px] text-[#888]">{targetName}</div>
              </div>
            </div>

            {adding ? (
              <div className="bg-[#1e1e1e] border border-lime rounded-sm p-4 mb-3">
                ...
                <div className="flex gap-2 mt-3">
                  ...
                  <button
                    ...
                    className="px-4 py-3 border border-[#333] rounded-sm font-mono text-[10px] text-[#777]"
                  >✕</button>
                </div>
              </div>
            ) : (
              <div>
                <button
                  ...
                  className="w-full py-3.5 border border-dashed border-[#333] rounded-sm font-mono text-[11px] uppercase tracking-widest text-[#777] mb-3"
                >+ Add intermediary</button>
                <div className="font-mono text-[9px] text-[#888] text-center mt-2 leading-relaxed">Someone you personally know who might be closer to your target.</div>
              </div>
            )}
